...
$(document).ready(function () {
        ...
        success: function (data) {
            if (data.success) {
                ...
                archivePeopleBodyEle.find('.chat__person-box-archive').
                    each(function () {
                        $(this).text(Lang.get('messages.new_keys.unarchive_chat'));
                    });
            ...
            success: function (data) {
                ...
                if (!isGroup && data.data.user.is_blocked_by_auth_user) {
                    ...
                    $('.blocked-message-text span').
                        text(Lang.get('messages.new_keys.you_are_blocked_by_this_user'));
                }
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    $('.request__content-title').
                        text(Lang.get('messages.new_keys.you_have_send_request_to_this_user'));
    ...
    $(document).on('click', '#reportUser', function () {
        ...
        if (reportUserNote === '') {
            $('#reportUserValidationErrorsBox').
                show().
                text(Lang.get('messages.new_keys.notes_required'));
            return false;
        }
    ...
    window.showConversationInfo = function (messageInfo) {
        ...
        if (remainingUsers.length > 0) {
            remainingUsersEle.text(remainingUsers.length + ' ' + Lang.get('messages.new_keys.remaining')).attr('data-remaining_count', remainingUsers.length);
...
// getting data on modal for search user
    ...
    window.fireSwal = function (
        ...
        Swal.fire({
            ...
            confirmButtonText:Lang.get('messages.new_keys.ok'),
            timer: 2000,
        });
    };
    ...
    window.deleteConversation = function (userId) {
        $.ajax({
            ...
            success: function (data) {
                ...
                fireSwal('success', Lang.get('messages.new_keys.Deleted'), Lang.get('messages.new_keys.conversation_deleted'));
    ...
    $(document).on('click', '.chat__person-box-archive', function (e) {
        ...
        let ArchiveUnarchive = (isArchiveChat) ? Lang.get('messages.new_keys.unarchive') : Lang.get('messages.new_keys.archive');

        swalDelete.fire({
            title: Lang.get('messages.are_you_sure'),
            html: ArchiveUnarchive + ' ' + Lang.get('messages.new_keys.chat_with')+' ' +'<b>' + contactName + '</b>?',
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: Lang.get('messages.new_keys.cancle'),
            confirmButtonText: Lang.get('messages.new_keys.yes'),
        }).then((result) => {
            if (result.value) {
                if(ArchiveUnarchive == Lang.get('messages.new_keys.unarchive')) {
    ...
    window.archiveConversation = function (userId) {
        $.ajax({
            ...
            success: function (data) {
                if (data.success == true) {
                    fireSwal('success', Lang.get('messages.new_keys.archived'),
                        Lang.get('messages.new_keys.conversation_has_been_archived'));

                    let archiveConversation = $('#user-' + userId);
                    $('#user-' + userId).remove();

                    archiveConversation.find('.chat__person-box-archive').
                        text(Lang.get('messages.new_keys.unarchive_chat'));
    ...
    window.unaArchiveConversation = function (userId) {
        $.ajax({
            ...
            success: function (data) {
                if (data.success == true) {
                    fireSwal('success', Lang.get('messages.new_keys.unarchived'),
                        Lang.get('messages.new_keys.conversation_unarchived'));
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success === true) {
                    ...
                    fireSwal('success', Lang.get('messages.new_keys.Deleted'),
                        Lang.get('messages.new_keys.message_has_been_deleted'));
    ...
    function updateGroupDetails (group) {
        ...
        if (!group.description) {
            $('#groupDetailsDescription-' + group.id).
                text(Lang.get('messages.new_key'));
        }
    ...
    $(document).on('click', '.msg-delete-for-everyone', function (e) {
        ...
        swalDelete.fire({
            title: Lang.get('messages.are_you_sure'),
            html:  Lang.get('messages.new_keys.delete_this_message'),
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: Lang.get('messages.new_keys.cancle'),
            confirmButtonText: Lang.get('messages.placeholder.yes_delete')
    ...
    function performActionsAfterUnblock (user) {
        ...
        if ($('#blockedUsersList').length <= 1) {
            ...
            $('.no-blocked-user').find('span').text(Lang.get('messages.no_users_blocked'));
        }
        ...
        if ($('.chat__person-box--active').data('id') == user.id) {
            appendChatArea();
            $('.block-unblock-span').text(Lang.get('messages.block'));
            $('.hdn-text-message').remove();
            $('.blocked-message-text').remove();
    ...
    function performActionAfterBlock (user) {
        $('.chat__area-text').remove();
        $('.blocked-message-text').remove();
        $('.block-unblock-span').text(Lang.get('messages.unblock'));
        $('.chat__area-wrapper').append(blockedMessageText);
    ...
    $(document).on('click', '.btn-unblock', function (e) {
        ...
        swalDelete.fire({
            title: Lang.get('messages.are_you_sure'),
            html: Lang.get('messages.new_keys.unblock_this_user'),
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: Lang.get('messages.new_keys.cancle'),
            confirmButtonText: Lang.get('messages.new_keys.yes'),
    ...
    $(document).on('click', '.block-unblock-user-switch', function (e) {
        ...
        let blockUserText = (isBlocked) ? Lang.get('messages.block') : Lang.get('messages.unblock');
        swalDelete.fire({
            title: Lang.get('messages.are_you_sure'),
            html: Lang.get('messages.new_keys.you_want_to') + blockUserText + Lang.get('messages.new_keys.this_user'),
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: Lang.get('messages.new_keys.cancle'),
            confirmButtonText: Lang.get('messages.new_keys.yes'),
    ...
    $(document).on('change', '#editGroupImage', function () {
        ...
        if ($.inArray(ext, ['gif', 'png', 'jpg', 'jpeg']) == -1) {
            ...
            $('#editGroupValidationErrorsBox').
                html(Lang.get('messages.new_keys.image_upload_valid')).
                show()
        } else {
    ...
    function performActionAfterUpdatingGroup (group) {
        ...
        if (!group.description) {
            $('#groupDetailsDescription-' + groupId).
                text(Lang.get('messages.new_keys.no_description_added_yet'));
        }
...
// Fill members to group members modal
    ...
    $(document).on('click', '.remove-member-from-group', function (e) {
        ...
        swalDelete.fire({
            title: Lang.get('messages.are_you_sure'),
            html: Lang.get('messages.new_keys.remove_member_from_this_group'),
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: Lang.get('messages.new_keys.cancle'),
    ...
    $(document).on('click', '.make-member-to-group-admin', function (e) {
        ...
        swalDelete.fire({
            title: Lang.get('messages.are_you_sure'),
            html: Lang.get('messages.new_keys.make_this_member_to_admin'),
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: Lang.get('messages.new_keys.cancle'),
            confirmButtonText: Lang.get('messages.new_keys.yes'),
    ...
    $(document).on('click', '.dismiss-admin-member', function (e) {
        ...
        swalDelete.fire({
            title: Lang.get('messages.are_you_sure'),
            html: Lang.get('messages.new_keys.dismiss_this_member_from_admin'),
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: Lang.get('messages.new_keys.cancle'),
            confirmButtonText: Lang.get('messages.new_keys.yes'),
    ...
    function prepareDeleteGroupButton (groupId) {
        return '<div class="chat-profile__column pt-1"> <a href="#" class="btn btn-danger btn-delete-group" data-group-id="' +
            groupId + '">'+ Lang.get('messages.group.delete_group') +'</a></div>';
    }
...
window.Dropzone.options.dropzone = {
    ...
    dictDefaultMessage: Lang.get('messages.new_keys.drop_files_here_to_upload'),
    uploadMultiple: true,
    init: function () {
