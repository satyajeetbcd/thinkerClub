        ...
        <div class="chat-profile__column">
            ...
            <p class="chat-profile__column-title-detail text-muted mb-0 group-desc" id="groupDetailsDescription-{{:id}}">
                {{if description}}
                    {{:description}}
                {{else}}
               <?php echo trans('messages.new_keys.no_description_added_yet') ?>
                {{/if}}
            </p>
        </div>
        <div class="chat-profile__divider"></div>
                                ...
                                <div class="chat__person-box-group" data-member-id="{{:id}}" data-group-id="{{:~root.id}}">
                                   <div class="btn-group">
                                      ...
                                      <div class="dropdown-menu member-options-{{:id}}">
                                        <a class="dropdown-item remove-member-from-group" href="#" data-member-id="{{:id}}" data-group-id="{{:~root.id}}" id="removeMember-{{:id}}">
                                            <?php echo __('messages.new_keys.remove_member') ?>
                                        </a>
                                       {{if pivot.role === 2}}
                                        <a class="dropdown-item dismiss-admin-member" href="#" data-member-id="{{:id}}" data-group-id="{{:~root.id}}" id="dismissAdmin-{{:id}}">
                                            <?php echo __('messages.new_keys.dismiss_as_admin') ?>
                                        </a>
                                        {{else}}
                                             <a class="dropdown-item make-member-to-group-admin" href="#" data-member-id="{{:id}}" data-group-id="{{:~root.id}}" id="makeAdmin-{{:id}}">
                                                <?php echo __('messages.new_keys.make_admin') ?>
                                             </a>
                                        {{/if}}
                                      </div>
                                    </div>
                    ...
                    <div class="chat-profile__column--media">
                        ...
                        <div class="chat-profile__media-container">
                            {{if media && media.length}}
                                ...
                                {{/for}}
                            {{else}}
                                <span class="no-photo-found text-muted"><?php echo __('messages.chats.no_media_share_yet') ?></span>
                            {{/if}}
                        </div>
                    </div>
                </div>
