<script id="tmplUserDetails" type="text/x-jsrender">
            ...
            <div class="ms-3">
                ...
                <p class="chat-profile__column-title-detail text-muted mb-0 user-about">
                     {{if about}}
                        {{:about}}
                    {{else}}
                    <?php echo trans('messages.new_keys.notes_required') ?>
                    {{/if}}
                </p>
            </div>
        </div>
                ...
                <div class="ms-3">
                    ...
                    <p class="chat-profile__column-title-detail text-muted mb-0 user-phone">
                        {{if phone}}
                            {{:phone}}
                        {{else}}
                            <?php echo trans('messages.new_keys.no_phone_added_yet') ?>
                        {{/if}}
                    </p>
                </div>
            </div>
    ...
    <div class="chat-profile__column chat-profile__column--media">
        ...
        <div class="chat-profile__media-container">
           {{if media && media.length}}
                ...
                {{/for}}
        {{else}}
            <span class="no-photo-found text-muted"><?php trans('messages.chats.no_media_share_yet')?></span>
        {{/if}}
        </div>
    </div>
    ...
    <div class="chat-profile__column">
        ...
        {{if common_groups.length === 0}}
            <p class="no-group-members-found text-center"> <?php trans('messages.new_keys.no_common_group') ?></p>
        {{/if}}
    </div>
    ...
    <div class="chat-profile__column">
        ...
        <div class="switch-checkbox chat-profile__switch-checkbox">
            ...
            <span class="chat-profile__column-title-detail text-muted mb-0 block-unblock-span"><?php echo __('messages.unblock') ?></span>
        </div>
        {{else}}
              <div class="switch-checkbox chat-profile__switch-checkbox">
            <input type="checkbox" id="switch" class="block-unblock-user-switch"/><label for="switch" class="mb-0 me-2">Toggle</label>
            <span class="chat-profile__column-title-detail text-muted mb-0 block-unblock-span"><?php echo __('messages.block') ?></span>
        </div>
        {{/if}}
    </div>
    <div class="chat-profile__divider"></div>
