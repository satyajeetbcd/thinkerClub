                ...
                <div class="chat-conversation__menu d-flex align-items-center">
                    <div class="dropdown btn-group hide-ele msg-options">
                        ...
                        <div class="dropdown-menu chat-replay-dropdown">
                            {{if allowToDelete}}
                                <a class="dropdown-item msg-delete-icon" href="#"><?php echo __('messages.new_keys.delete_message'); ?></a>
                            {{/if}}
                            {{if deleteMsgForEveryone}}
                                <a class="dropdown-item msg-delete-for-everyone" href="#">
                                    <?php echo __('messages.new_keys.delete_for_everyone'); ?></a>
                            {{/if}}
                             <a class="dropdown-item msg-replay" href="#" data-self-reply="1" data-message-id="{{:data.id}}" data-message='{{:data.message}}' data-message-type='{{:data.message_type}}'>
                                <?php echo __('messages.new_keys.reply') ?>
                             </a>
                             <a class="dropdown-item open-msg-info" data-message-id="{{:data.id}}" data-is_group="{{:data.is_group}}">
                             <?php echo __('messages.new_keys.info') ?>
                             </a>
                        </div>
                    </div>
                </div>
         ...
         <div class="chat-conversation__menu d-flex align-items-center">
            <div class="dropdown btn-group hide-ele msg-options">
                ...
                <div class="dropdown-menu">
                    <a class="dropdown-item msg-replay" href="#" data-self-reply="0" data-message-id="{{:data.id}}" data-message='{{:data.message}}' data-sender="{{:data.sender.name}}" data-user-id="{{:data.sender.id}}" data-message-type='{{:data.message_type}}'>
                        <?php echo __('messages.new_keys.reply') ?>
                    </a>
                </div>
            </div>
